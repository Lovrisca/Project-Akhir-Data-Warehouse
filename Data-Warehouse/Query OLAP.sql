...
FROM ETL_DW_FIX.dbo.FactSales FS
JOIN ETL_DW_FIX.dbo.DimDate D ON D.IDDate = FS.IDDate
JOIN ETL_DW_FIX.dbo.DimProduct P ON P.IDProduct = FS.IDProduct
JOIN ETL_DW_FIX.dbo.DimCustomer C ON C.IDCustomer = FS.IDCustomer
JOIN ETL_DW_FIX.dbo.DimSalesTerritory ST ON ST.IDSalesTerritory = FS.IDSalesTerritory
WHERE D.Year = 2011
AND D.Month = 7
AND ST.Country = 'US'
...
SELECT DENSE_RANK() OVER (ORDER BY SUM(FS.SalesAmount) DESC) AS Ranking,
